...
// Default constructor.
TcpBbrAdaptive::TcpBbrAdaptive(void) :
  TcpCongestionOps(),
  m_pacing_gain(0.0),
  m_cwnd_gain(0.0),
  m_round(0),
  ...
  NS_LOG_INFO(this << "  BbrAdaptive' version: v" << BbrAdaptive::VERSION);

  // Constants in "tcp-Bbr-Adaptive.h"
  NS_LOG_INFO(this << "  INIT_RTT: " << BbrAdaptive::INIT_RTT.GetSeconds() << " sec");
  NS_LOG_INFO(this << "  INIT_BW: " << BbrAdaptive::INIT_BW << " Mb/s");
  NS_LOG_INFO(this << "  BW_WINDOW_TIME: " << BbrAdaptive::BW_WINDOW_TIME << " rtts");
  NS_LOG_INFO(this << "  MIN_CWND: " << BbrAdaptive::MIN_CWND << " bytes");
  NS_LOG_INFO(this << "  STARTUP_THRESHOLD: " << BbrAdaptive::STARTUP_THRESHOLD);
  NS_LOG_INFO(this << "  STARTUP_GAIN: " << BbrAdaptive::STARTUP_GAIN);
  NS_LOG_INFO(this << "  STEADY_FACTOR: " << BbrAdaptive::STEADY_FACTOR);
  NS_LOG_INFO(this << "  PROBE_FACTOR: " << BbrAdaptive::PROBE_FACTOR);
  NS_LOG_INFO(this << "  DRAIN_FACTOR: " << BbrAdaptive::DRAIN_FACTOR);
  NS_LOG_INFO(this << "  PACING_FACTOR: " << BbrAdaptive::PACING_FACTOR);

  // Timing config (used for culling BW window) in "tcp-BbrAdaptive.h"
  if (BbrAdaptive::TIME_CONFIG == BbrAdaptive::WALLCLOCK_TIME) 
    NS_LOG_INFO("TIME_CONFIG: WALLCLOCK_TIME - BW window culling with wallclock time.");
  else if (BbrAdaptive::TIME_CONFIG == BbrAdaptive::PACKET_TIME) 
    NS_LOG_INFO("TIME_CONFIG: PACKET_TIME - BW window culling with packet time.");
  else  {
    NS_LOG_INFO("WARNING! Unknown TIME_CONFIG: " << BbrAdaptive::TIME_CONFIG);
    NS_LOG_INFO("BW window culling with packet time.");
  }

  // Constant in "tcp-socket-base.h"
  if (PACING_CONFIG == NO_PACING) 
    NS_LOG_INFO(this << "  Note: BbrAdaptive' configured with pacing NO_PACING.");
...
// Get type id.
TypeId TcpBbrAdaptive::GetTypeId(void) {
...
std::string TcpBbrAdaptive::GetName() const {
  NS_LOG_FUNCTION(this);
  return "TcpBbrAdaptive";
}

// Copy BbrAdaptive' congestion control algorithm across socket.
Ptr<TcpCongestionOps> TcpBbrAdaptive::Fork() {
  NS_LOG_FUNCTION(this);
...
void TcpBbrAdaptive::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segs_acked) {
...
uint32_t TcpBbrAdaptive::GetSsThresh(Ptr<const TcpSocketState> tcb,
...
// tcb = transmission control block
void TcpBbrAdaptive::PktsAcked(Ptr<TcpSocketState> tcb, uint32_t packets_acked,
                       const Time &rtt) {
  ...
  BbrAdaptive::packet_struct packet;
  ...
  if (m_pacing_gain == 1)
    pacing_rate *= BbrAdaptive::PACING_FACTOR;
  ...
  if (PACING_CONFIG != NO_PACING) {
    ...
    if (m_machine.getStateType() == BbrAdaptive::PROBE_RTT_STATE) {
      double probe_rtt_pacing_rate = BbrAdaptive::MIN_CWND;   // Bytes (B).
      ...
      NS_LOG_LOGIC(this << " In PROBE_RTT," <<
                   "  BbrAdaptive::MIN_CWND: " << BbrAdaptive::MIN_CWND <<
...
// tsb = tcp socket base
// tcb = transmission control block
void TcpBbrAdaptive::Send(Ptr<TcpSocketBase> tsb, Ptr<TcpSocketState> tcb,
...
double TcpBbrAdaptive::getBW() const {
  double max_bw = 0;
...
Time TcpBbrAdaptive::getRTT() const {
...
double TcpBbrAdaptive::getBDP() const {
  NS_LOG_FUNCTION(this);
  Time rtt = getRTT();
  if (rtt.IsNegative())
    rtt = BbrAdaptive::INIT_RTT;
  double bw = getBW();
  if (bw < 0)
    bw = BbrAdaptive::INIT_BW;
  return (double) (rtt.GetSeconds() * bw);
}

// Remove BW estimates that are too old (greater than 10 RTTs).
void TcpBbrAdaptive::cullBWwindow() {
  ...
  Time now = Simulator::Now();
  Time time_delta = now - rtt * BbrAdaptive::BW_WINDOW_TIME;
  int round_delta = m_round - BbrAdaptive::BW_WINDOW_TIME;
  ...
  while (it != m_bw_window.end()) {
    ...
    if (BbrAdaptive::TIME_CONFIG == BbrAdaptive::WALLCLOCK_TIME) { // Use wallclock time.
...
void TcpBbrAdaptive::cullRTTwindow() {
  ...
  Time now = Simulator::Now();
  Time delta = Time(now - BbrAdaptive::RTT_WINDOW_TIME * 1000000000.0); // Units are nanoseconds.
...
bool TcpBbrAdaptive::checkProbeRTT() {

  NS_LOG_FUNCTION(this);

  // Check if PROBE_BW and min RTT hasn't changed in 10 seconds.
  Time now = Simulator::Now();
  if (m_machine.getStateType() == BbrAdaptive::PROBE_BW_STATE &&
      (now.GetSeconds() - m_min_rtt_change.GetSeconds()) >
      BbrAdaptive::RTT_NOCHANGE_LIMIT) {
...
// tcb = transmission control block
void TcpBbrAdaptive::CongestionStateSet(Ptr<TcpSocketState> tcb,
...
void TcpBbrAdaptive::updateTargetCwnd() {
  ...
  if (m_cwnd < BbrAdaptive::MIN_CWND) {
    NS_LOG_LOGIC(this << "  m_cwnd (bytes): " << m_cwnd <<
                 "  Boosting to (bytes): " << BbrAdaptive::MIN_CWND);
    m_cwnd = BbrAdaptive::MIN_CWND; // In bytes.
  }
