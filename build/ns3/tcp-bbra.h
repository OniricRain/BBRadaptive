 ...
 *
 * \brief Implementation of basic TCP bbra' functionality.
 *
 */
class Tcpbbra : public TcpCongestionOps {

public:

  friend class bbraState;
  friend class bbraStartupState;
  friend class bbraDrainState;
  friend class bbraProbeBWState;
  friend class bbraProbeRTTState;
  friend class bbraStateMachine;
  ...
  // Copy bbra' congestion control with copy.
  virtual Ptr<TcpCongestionOps> Fork();

  // bbra' ignores calls to increase window.
  virtual void IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segs_acked);

  // bbra' does not use ssthresh, so ignored.
  virtual uint32_t GetSsThresh(Ptr<const TcpSocketState> tcb, uint32_t b_in_flight);

  // Events/calculations specific to bbra' congestion state.
...
private:
 ...
 protected:
  ...
  bbraStateMachine m_machine;               // State machine.
  bbraStartupState m_state_startup;         // STARTUP state.
  bbraDrainState m_state_drain;             // DRAIN state.
  bbraProbeBWState m_state_probe_bw;        // PROBE_BW state.
  bbraProbeRTTState m_state_probe_rtt;      // PROBE_RTT state.
};
